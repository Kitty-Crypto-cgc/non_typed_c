...
public:
    ...
    void assign_object(String in_object_name = "NULL")
    {
        if (in_object_name != "NULL")
        ...
        id_giver.assign_id(&object_id, &object_name, &finished);
    }
    ...
    bool are_compatibile(basic_input *input_1, basic_input *input_2)
    {
        bool result = false;
    ...
    {
        String id_1, id_2, error_message;
        char in1_type, in2_type;

        id_1 = int_to_str(input_1->get_id());
        id_2 = int_to_str(input_2->get_id());

        in1_type = input_1->get_datatype();
        in2_type = input_2->get_datatype();

        error_message = "Cannot perform " + operation + " of the " + print_datatype(in2_type, true, false) + " with ID:" + id_2 + ", with the " + print_datatype(in1_type, true, false) + " with ID: " + id_1;
    ...
    {
        String my_id, other_id, error_message;
        char my_type, other_type;

        my_id = int_to_str(get_id());
        other_id = int_to_str(other_input->get_id());

        my_type = get_datatype();
        other_type = other_input->get_datatype();

        error_message = "Cannot perform " + operation + " of the " + print_datatype(other_type, true, false) + " with ID: " + other_id + ", with the " + print_datatype(my_type, true, false) + " with ID: " + my_id;
        print_error(error_message, add_endl);
    }

    void print_not_init_error(String operation, bool add_endl = true)
    {
        String my_id, error_message;
        my_id = int_to_str(object_id);
        error_message = "Cannot perform " + operation + " operation. Object " + my_id + " is not initialised";
        print_error(error_message, add_endl);
    }
