...
public:
    ...
    void change_text(String in_text)
    {
        if (!is_locked())
            ...
            print_error("Error changing text: Object locked");
    }
...
public:
    ...
    {
        latest_id = 0;
        object_names = new String[latest_id];
        initialised = true;
    }

    void assign_id(Ulint *input_id, String *in_object_name, bool *finished)
    {
        if (!is_locked())
        {
            if (!*finished)
            {
                ...
                for (Ulint i = 0; i < latest_id; i++)
                {
                    if (object_names[i] == *in_object_name)
                    {
                        name_used = true;
                        break;
                    }
                ...
                if (name_used)
                {
                    *in_object_name = *in_object_name + "_new";
                    assign_id(input_id, in_object_name, finished);
                }
                else
                    *finished = true;
            }
            ...
            {    
                *input_id = latest_id;
                String *new_object_names;
                new_object_names = new String[latest_id];

                for (Ulint i = 0; i < latest_id; i++)
                    new_object_names[i] = object_names[i];

                delete[] object_names;

                object_names = new String[latest_id + 1];

                for (Ulint i = 0; i < latest_id; i++)
                    object_names[i] = new_object_names[i];

                delete[] new_object_names;

                object_names[latest_id] = *in_object_name;
        ...
        {
            print_error("Error assigning ID: ID giver is locked so no more IDs can be assigned");
        }
    }
};

class multitype : public ID
{
protected:
    enum Tern : char
    {
