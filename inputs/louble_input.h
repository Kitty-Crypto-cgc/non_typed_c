...
protected:
    ...
    bool operation_checks(basic_input *address_other_op, String operation)
    {
        if (!is_initialised())
        {
        ...
        if (!are_compatibile(address_other_op))
        {
            print_incompatible_error(address_other_op, operation);
            return false;
        }
        return true;
    }
...
private:
    ...
    {
        if ((buffer <= std::numeric_limits<MYTYPE>::max()) && (buffer >= -1 * std::numeric_limits<MYTYPE>::min()))
            return true;
        else
        {
            if (mute)
...
public:
    ...
    MYMETATYPE operator+(basic_input other_louble)
    {
        MYMETATYPE temp1(input.value.louble_);
        MYMETATYPE temp2(other_louble.cast_to_double());
